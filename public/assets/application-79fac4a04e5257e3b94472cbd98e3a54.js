...
(function() {
  $(function() {
    $('#adminTabs').on('shown.bs.tab', function(e) {
      var target;
      target = $(e.target).attr("href");
      return $.get("/admin/" + target.replace(/#/, ''), (function(data) {
        $("#adminContent").html(data);
        return $("#content").trigger("refreshProducts");
      }), 'html');
    });
    $('#adminTabs a:first').tab('show');
    $("#content").on("ajax:complete", ".remove", function() {
      return alert("success");
    });
    $("#content").on("ajax:complete", ".edit_special", function(data, status, xhr) {
      return $(this).find(".saveStatus:first").text(status.responseText);
    });
    $("#content").on("ajax:complete", ".newSpecial", function(data, status, xhr) {
      return $("#specialsTable tbody").append("<tr><td>" + status.responseText);
    });
    return $("#content").on("refreshProducts", function() {
      if ($(".updated").length === 1) {
        return $(".prodContainer:first").scrollTop($(".updated:first").offset().top - $(".prodContainer:first").offset().top);
      }
    });
  });
...
(function() {
  jQuery(function() {
    ...
    return $(".shipping").click(function() {
      if ($(this).is(':checked')) {
        return $("#new_shipping_address").show();
      } else {
        return $("#new_shipping_address").hide();
      }
    });
  });
...
(function() {
  $(function() {
    $("#catfilter").on('change', '.catfilter', function() {
      var p;
      p = {
        cats: []
      };
      $(".catfilter:checked").each(function() {
        ...
        return $("#catSelectAll").attr("checked", false);
      });
      return $.get('/products/partIdx', p, (function(data) {
        return $("#productList").html(data);
      }), 'html');
    });
    $("#catSelectAll").click(function() {
      $(".catfilter").prop("checked", this.checked);
      return $.get('/products/partIdx', [], (function(data) {
        return $("#productList").html(data);
      }), 'html');
    });
    return $("#content").on("ajax:complete", ".editProduct", function(data, status, xhr) {
      return $("#myModal").html(status.responseText).modal();
    });
  });
...
(function() {
  $(function() {
    return $("#content").on("ajax:complete", ".new_user", function(data, status, xhr) {
      var json;
